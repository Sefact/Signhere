...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			...
			<table class="table table-striped">
				...
				<tbody id = "docListBody">
					<c:forEach var="docList" items="${docList}">
						<tr class = "waitDocListRow" id="${docList.dmNum }" >
		...
		<ul class="btn-group pagination">
				    ...
				    <li>
				        <a href='<c:url value="/apToDoList?page=${pagination.startPage-1 }"/>'><i class="fa fa-chevron-left"></i></a>
				    </li>
				    </c:if>
				    <c:forEach begin="${pagination.startPage }" end="${pagination.endPage }" var="pageNum">
				    <li>
				        <a href='<c:url value="/apToDoList?page=${pageNum }"/>'><i class="fa">${pageNum }</i></a>
				    </li>
				    </c:forEach>
				    <c:if test="${pagination.next && pagination.endPage >0 }">
				    <li>
				        <a href='<c:url value="/apToDoList?page=${pagination.endPage+1 }"/>'><i class="fa fa-chevron-right"></i></a>
				    </li>
				    </c:if>
		</ul>
	</div>
